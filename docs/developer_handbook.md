...
- as a result, they can are available in our tests and other modules
...
- we need to add all properties of the record, even if they are not consumed yet, because the
  template is defined `strict`
- we don't need to adjust the test cases because they are generated based on the `RecordValue`
  interface (i.e. if a property is not in the interface then it is not covered by the tests)
